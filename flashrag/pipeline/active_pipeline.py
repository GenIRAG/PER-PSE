...
from transformers import AutoTokenizer, PreTrainedTokenizer, PreTrainedTokenizerFast, AutoModelForSeq2SeqLM
...
from flashrag.utils.utils import print_now, set_logger, str2json
...
class SelfAskPipeline(BasicPipeline):
    ...
    def __init__(self, config, prompt_template=None, max_iter=5, single_hop=True):
        ...
        self.single_hop = config["selfask_params"].get("single_hop", single_hop)
        self.max_iter = config["selfask_params"].get("max_iter", max_iter)
        self.P_INS = SELF_ASK_PROMPT_SINGLE_HOP if self.single_hop else SELF_ASK_PROMPT_MULTI_HOP
        self.follow_up_pattern = config["selfask_params"].get("follow_up_pattern", self.FOLLOW_UP_PATTERN)
    ...
    def run_item(self, item):
        ...
        output_item['reference'] = retrieval_result
        ...
        for idx in range(self.max_iter):
            ...
            try:
                gen_out = self.generator.generate(input_list=input_prompt if self.framework == "hf" else input_list, stop=["Context:", "#", stop_condition])[0]
            except Exception as e:
                print(f"Error during generation: {e}")

            print(f"iter {idx + 1} gen out is: {gen_out}")
            ...
            print(f"iter {idx + 1} result is: {res}")
            ...
            if "Follow up: " in gen_out:
                ...
                logs[f"iter{idx + 1}"] = {"output": gen_out, "reference": retrieval_result}
            else:
                logs[f"iter{idx + 1}"] = {"output": gen_out}
        ...
        output_item['intermediate_log'] = logs
        cleaned_final_res = selfask_pred_parse(final_res)
        output_item['final_result'] = cleaned_final_res

        item.update_output("pred", final_res)
        item.update_output("process_log", output_item)
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=selfask_pred_parse):
        ...
        set_logger(self.config, info=[f"[{print_now(1)}] evaluate result: {eval_result}"])
...
class IRCOTPipeline(BasicPipeline):
    def __init__(self, config, prompt_template=None, retriever=None, generator=None, max_iter=2):
        IRCOT_RETRIEVE_SYSTEM_PROMPT = """You serve as an intelligent assistant, adept at facilitating users through complex, multi-hop reasoning across multiple documents. This task is illustrated through demonstrations, each consisting of a document set paired with a relevant question and its multi-hop reasoning thoughts. Your job is to complete the Thought one step at a time, and the step can't be the same as the previous steps. Once step is enough, answer a short phrase or a few words immediately, the answer must be start with "So the answer is:".\n"""
        IRCOT_REASON_SYSTEM_PROMPT = """You are a question answering system. Use the evidence while generating the answer and keep the answer grounded in the evidence. 
        Directly generate a JSON with a single key "Response" and a value that is a short phrase or a few words. In JSON, put every value as a string always, not float."""

        from flashrag.pipeline import IRCOT_PROMPT, IRCOT_QA_PROMPT

        prompt_chooser = config["split"][0]

        if prompt_chooser == "2hop_bridge":
            IRCOT_RETRIEVE_EXAMPLES = IRCOT_PROMPT._2HOP_BRIDGE_PROMPT
            IRCOT_REASON_EXAMPLES = IRCOT_QA_PROMPT._2HOP_BRIDGE_QA_PROMPT
        elif prompt_chooser == "2hop_comparison":
            IRCOT_RETRIEVE_EXAMPLES = IRCOT_PROMPT._2HOP_COMPARISON_PROMPT
            IRCOT_REASON_EXAMPLES = IRCOT_QA_PROMPT._2HOP_COMPARISON_QA_PROMPT
        elif prompt_chooser == "2hop_inference":
            IRCOT_RETRIEVE_EXAMPLES = IRCOT_PROMPT._2HOP_INFERENCE_PROMPT
            IRCOT_REASON_EXAMPLES = IRCOT_QA_PROMPT._2HOP_INFERENCE_QA_PROMPT
        elif prompt_chooser == "4hop_comparison":
            IRCOT_RETRIEVE_EXAMPLES = IRCOT_PROMPT._4HOP_COMPARISON_PROMPT
            IRCOT_REASON_EXAMPLES = IRCOT_QA_PROMPT._4HOP_COMPARISON_QA_PROMPT
        elif prompt_chooser == "4hop_bridge_comparison":
            IRCOT_RETRIEVE_EXAMPLES = IRCOT_PROMPT._4HOP_BRIDGE_COMPARISON_PROMPT
            IRCOT_REASON_EXAMPLES = IRCOT_QA_PROMPT._4HOP_BRIDGE_COMPARISON_QA_PROMPT
        elif prompt_chooser == "3hop_bridge":
            IRCOT_RETRIEVE_EXAMPLES = IRCOT_PROMPT._3HOP_BRIDGE_PROMPT
            IRCOT_REASON_EXAMPLES = IRCOT_QA_PROMPT._3HOP_BRIDGE_QA_PROMPT
        elif prompt_chooser == "4hop_bridge":
            IRCOT_RETRIEVE_EXAMPLES = IRCOT_PROMPT._4HOP_BRIDGE_PROMPT
            IRCOT_REASON_EXAMPLES = IRCOT_QA_PROMPT._4HOP_BRIDGE_QA_PROMPT
        ...
        self.ircot_retrieve_prompt_template = PromptTemplate(
            config=config,
            system_prompt=IRCOT_RETRIEVE_SYSTEM_PROMPT+IRCOT_RETRIEVE_EXAMPLES,
            user_prompt="{reference}Question: {question}\nThought: Complete the next step! ",
            reference_template="Wikipedia Title: {title}\n{text}\n\n",
            enable_chat=True
        )

        self.ircot_reason_prompt_template = PromptTemplate(
            config=config,
            system_prompt=IRCOT_REASON_SYSTEM_PROMPT+IRCOT_REASON_EXAMPLES,
            user_prompt="{reference}Question: {question}\nAnswer: ",
            reference_template="Wikipedia Title: {title}\n{text}\n\n",
            enable_chat=True
        )
    ...
    def run_item(self, item):
        ...
        intermediate_logs["retrieval_result"] = retrieval_result
        intermediate_logs["sorted_first_retrieval_result"] = copy.deepcopy(doc2score)
        thoughts = []
        iter_num = 0
        intermediate_logs["process_logs"] = {}
        while iter_num < self.max_iter:
            process_log = {}

            input_prompt = [self.ircot_retrieve_prompt_template.get_string(
                question=question, retrieval_result=retrieval_result[-15:], previous_gen=" ".join(thoughts)
            )]


            new_thought = self.generator.generate(input_prompt, stop=['\n'], max_tokens=128)[0]
            new_thought_split = self.nlp(new_thought)
            new_thought = list(new_thought_split.sents)[0].text
            # new_thought = new_thought.split("\n")[0]
            process_log["thought"] = new_thought

            thoughts.append(new_thought)
            iter_num += 1
            if "So the answer is:" in new_thought:
                intermediate_logs["process_logs"][f"{iter_num}"] = process_log
                break
            ...
            if not contains_letters(new_thought) or len(new_thought.split(" ")) <= 3:
                break

            if "loop" not in new_thought.lower():
                ...
                process_log["retrieval_result"] = new_retrieval_result
                process_log["sorted_all_retrieval_result"] = {f"{x}": y for x, y in sorted_doc_score}

            intermediate_logs["process_logs"][f"iter_{iter_num}"] = process_log

            if max(dict(Counter(thoughts)).values()) >= 3:
                break

        intermediate_logs["ircot_retrieve_cot"] = thoughts
        item.update_output("intermediate_logs", intermediate_logs)

        input_prompt = [self.ircot_reason_prompt_template.get_string(
            question=question, retrieval_result=retrieval_result[-15:]
        )]

        ircot_reason_direct = self.generator.generate(input_prompt, max_tokens=2048)[0]

        try:
            pred = str2json(ircot_reason_direct)["Response"]
        except:
            pred = ""

        item.update_output("ircot_reason_direct", ircot_reason_direct)
        item.update_output("pred", pred)
        return item
